...
const EditExpenseForm = ({ handleSubmitUpdateExpense, handleChange, setShowUpdate, formData }) => {
  return (  
    <>
      <div className={styles.overlay} onClick={() => setShowUpdate(false)}></div>
      <form 
        onSubmit={handleSubmitUpdateExpense}
        className={styles.form}
      >
        <button onClick={() => setShowUpdate(false)} className={styles['hide-btn']}>
        ...
        />
        <button className={styles['create-expense-btn']}>Save</button>
      </form>
    </>
  )
}
